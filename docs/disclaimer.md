...
Passclip is a personal project. It is not developed, maintained, or endorsed by any company, security organization, or the original authors of GNU `pass`. It is not audited by a third-party security firm. Use it at your own risk.
...
Passclip is built on top of GNU `pass` and GPG. It does not implement its own cryptography for the password store. The security of your passwords depends on:

- **Your GPG key** — if your private key is compromised or your passphrase is weak, your passwords are exposed. Passclip can't protect against that.
...
When you copy a password to the clipboard, it is briefly accessible to every application running on your system. Passclip auto-clears the clipboard after a timeout, but there is a window where other programs could read it. This is a fundamental limitation of how clipboards work on every major operating system — it is not a bug in Passclip.

## Not a substitute for good security practices

Passclip makes it easier to use strong, unique passwords. But a tool is only part of the picture. You should also:
...
- Regularly audit your passwords with `passclip health`
- Understand what GPG does and doesn't protect against

## Third-party dependencies

Passclip relies on GPG, `pass`, and several Python libraries (`rich`, `cryptography`, `pyperclip`, `pyotp`). Vulnerabilities in any of these could affect Passclip. Keep them updated.
...
*By using Passclip, you acknowledge that you have read and understood this disclaimer.*
