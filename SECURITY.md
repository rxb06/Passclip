...
- **Vault import hardening**: tar extraction rejects symlinks, hardlinks, absolute paths, null bytes, any member that is not a regular file or directory (no FIFOs or device nodes), and any member whose resolved path escapes the **password store itself** (checked via `Path.is_relative_to()` against the store directory, not merely its parent, and not string prefix matching). Member permission bits are clamped to `0o755` — no setuid/setgid/sticky, no group/other write. On top of those always-on checks, `filter='data'` is applied whenever the running Python supports it (3.12+, plus the 3.10.12+/3.11.4+ backports) as defense in depth. Vault passphrases must be at least 12 characters; weak ones require explicit confirmation.
- **File permissions**: config files, history files, vault exports, and pre-delete backups are all created with `0600` permissions (owner read/write only). The `~/.config/passclip/` and `backups/` directories are kept at `0700`, so other local users cannot list backup filenames (which embed entry paths).
- **Vault passphrase attempts**: vault import allows 3 passphrase attempts per invocation. In-tool delays are deliberately not used — an attacker holding the vault file brute-forces it offline with their own code, so the real resistance is PBKDF2-SHA256 at 600,000 iterations combined with the 12-character passphrase floor.
...
- **Concurrent access**: the interactive shell uses `fcntl.flock()` to prevent concurrent instances from corrupting state. The lock file lives in the `0700` config directory (not inside the git-synced store, so a malicious remote cannot plant a symlink at its path) and is opened with `O_NOFOLLOW`.
...
- **Optional dependencies handle live secrets**: `pyperclip` and `pyotp` are optional, but when installed they are the most security-relevant third-party code in the process — pyperclip sees every copied password, and pyotp receives raw TOTP seeds (not just the 6-digit codes). Install and update them with the same care as Passclip itself; version floors are pinned in `pyproject.toml`.
...
The interactive shell stores command history at `~/.config/passclip/history`. This file contains command names and entry paths (like `get email/gmail`) but **not** secrets: password and OTP-secret prompts are hidden (they bypass readline entirely), and the username/email/URL/notes prompts are excluded from readline history. It's created with `0600` permissions in a `0700` directory. If this concerns you, you can disable history persistence by locking the file — the shell detects this and skips history without breaking:
